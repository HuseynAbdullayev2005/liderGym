...
function SortableItem({ exercise, index, onInputChange }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="exercise_card">
      <div className="drag_handle" {...attributes} {...listeners}>
        &#9776;
      </div>
      {exercise.img && (
        <img src={exercise.img} alt={exercise.name} className="exercise_image_left" />
      )}
      <div className="exercise_right">
        <div className="exercise_muscle">Targeted Muscle: {exercise.muscle}</div>
        <h3>{exercise.name}</h3>
        <div className="exercise_inputs">
          <input
            type="number"
            placeholder="Weight"
...
const strengthExercises = {
  chest: [
    ...
    { name: "Dumbbell Fly", img: "/dumbellfly.jpg" },
    { name: "Incline Bench Press", img: "/inclinepress.jpg" }
  ],
  back: [
...
const strengthSplits = {
  ...
  fullBody: ["chest", "back", "shoulders", "biceps", "triceps", "quads", "hamstrings", "calves"]
};
    ...
    <>
      ...
      <div className="programs_container">
        ...
        <div className="programs_grid">
          <div className="program_card" onClick={() => handleProgramClick("strength")}>
            <i className="fas fa-dumbbell card_icon" />
            <h3>Strength Training</h3>
            <p>Train to improve your strength with multiple splits.</p>
          </div>
          <div className="program_card" onClick={() => handleProgramClick("bmi")}>
            <i className="fas fa-heartbeat card_icon" />
        ...
        <div className="program_content">
          {selectedProgram === "strength" && (
            <div className="strength_section">
              ...
              {selectedSplit && (
                <div className="workout_container">
                  ...
                  <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                    <SortableContext items={workout.map((ex) => ex.id)} strategy={verticalListSortingStrategy}>
                      <div className="exercises_list">
                        {workout.map((exercise, index) => (
                          <SortableItem key={exercise.id} exercise={exercise} index={index} onInputChange={handleInputChange} />
                        ))}
                      </div>
                    </SortableContext>
                  </DndContext>
          ...
          {selectedProgram === "bmi" && (
            <div className="fitness_calc">
              ...
              <div className="input_row">
                ...
                <input type="number" value={bench} onChange={(e) => setBench(e.target.value)} />
              </div>
              <div className="input_row">
              ...
              <button className="calc_btn" onClick={calculateMetrics}>Calculate</button>
              {bmi && (
              ...
              {strengthRatio && (
                <p className="result_text">
                  Strength Ratio (Bench + Squat + Deadlift) / Bodyweight = <strong>{strengthRatio}</strong>
                </p>
              )}
            </div>
          )}
